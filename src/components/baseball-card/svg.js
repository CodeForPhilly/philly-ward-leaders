...
export function createFront(el, data) {
  ...
  // Card
  svg.rect(5, 5, config.cardWidth - 10, config.cardHeight - 10, 2).attr({
  ...
  const photo = svg
    .image(data.photo, photoX, photoY, photoWidth, photoHeight)
    .attr({ preserveAspectRatio: photoRatio });

  const photoPattern = photo.toPattern().attr({
  ...
  svg
    .polygon([
      config.cardWidth - config.cardBorderWidth,
      ...
      config.cardWidth - config.bottomRightCut,
      config.cardHeight - config.cardBorderWidth,
      config.cardWidth - config.cardBorderWidth,
      config.cardHeight - config.bottomRightCut,
    ])
    .attr({
      fill: photoPattern,
      stroke: "#000",
  ...
  // Ward circle
  svg.circle(45, 45, 25).attr({
    fill: "#fff",
    stroke: "#000",
  });

  // Ward text line 1
  svg.text(45, 40, "Ward").attr({
    "text-anchor": "middle",
    "font-family": "arial",
    "font-size": 12,
    fill: config.cardBorderColor,
  ...
  // Ward text line 2
  svg.text(45, 58, data.ward + data.subWard).attr({
    "text-anchor": "middle",
    "font-family": "arial",
    "font-weight": "bold",
    "font-size": 18,
  ...
  svg
    .rect(nameCtrX, nameCtrY, nameCtrWidth, nameCtrHeight)
    .attr({
      fill: "#fff",
      stroke: "#000",
      strokeWidth: 1,
...
export function createBack(el, contentsEl, data) {
  ...
  svg.rect(insideX, insideY, insideWidth, insideHeight).attr({
